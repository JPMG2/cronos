...
<div class="flex h-full flex-col"
    ...
    <div class="flex min-h-0 flex-1 flex-col overflow-y-auto">
        ...
        @if($this->regions->isNotEmpty())
            <x-list.header :code="true" code-label="Prov." />
        @endif
        ...
        @forelse($this->regions as $region)
            <div wire:key="region-{{ $region->id }}"
                ...
                <div class="flex min-w-0 flex-1 items-center gap-2">
                    <span class="shrink-0 min-w-[52px] max-w-[120px] truncate rounded-lg bg-indigo-100 px-2 py-0.5 font-mono text-[10px] font-bold uppercase tracking-wider text-indigo-700 dark:bg-indigo-500/15 dark:text-indigo-300">
